"""
Scrape comments from an Instagram account's posts published in the last 30 days,
using the apify/instagram-scraper actor, and write them to a CSV file.

Usage:
    export APIFY_API_TOKEN=...
    python scrape_ig_comments.py <handle>
"""
...
def fetch_posts(client: ApifyClient, profile_url: str, since_date: date) -> list[dict]:
...
def main() -> None:
    if len(sys.argv) != 2:
        sys.exit("Usage: python scrape_ig_comments.py <handle>")
    ...
    print(f"[{handle}] Fetching posts from {start_date} to {end_date}...")
    posts = fetch_posts(client, profile_url, start_date)
    print(f"[{handle}] Found {len(posts)} post(s).")

    if not posts:
        print(f"[{handle}] No posts found in the given date range; nothing to scrape comments for.")
        return

    all_comments = []
    for i, post in enumerate(posts, start=1):
        print(f"[{handle}] Fetching comments for post {i}/{len(posts)}: {post['post_url']}")
        all_comments.extend(fetch_comments(client, post["post_url"]))
    print(f"[{handle}] Found {len(all_comments)} comment(s) total.")

    os.makedirs(OUTPUT_DIR, exist_ok=True)
    output_path = os.path.join(
        OUTPUT_DIR, f"{handle}_comments_{start_date.isoformat()}_{end_date.isoformat()}.csv"
    )
    write_csv(posts, all_comments, output_path)
    print(f"[{handle}] Wrote {len(all_comments)} comment(s) across {len(posts)} post(s) to {output_path}")
